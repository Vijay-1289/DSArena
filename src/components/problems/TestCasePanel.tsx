  ...
  return (
    <div className="flex h-full flex-col rounded-lg border border-border bg-card">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex h-full flex-col">
        ...
        <TabsContent value="testcases" className="flex-1 p-0 mt-0 overflow-hidden">
          <ScrollArea className="h-full w-full">
            <div className="space-y-4 p-4 pb-8">
              {visibleCases.map((testCase, index) => (
                <div
                  key={testCase.id}
                  className="rounded-lg border border-border bg-muted/30 p-4 break-words"
                >
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <pre className="rounded bg-background p-2 font-mono text-sm text-foreground whitespace-pre-wrap overflow-x-auto">
                        {testCase.input}
                      </pre>
                    </div>
                    <div>
        ...
        <TabsContent value="results" className="flex-1 p-0 mt-0 overflow-hidden">
          <ScrollArea className="h-full w-full">
            <div className="space-y-4 p-4 pb-8">
              {isRunning ? (
                ...
                results.map((result, index) => (
                  <div
                    key={index}
                    className={cn(
                      'rounded-lg border p-4 break-words',
                      result.passed
                  ...
                  >
                    ...
                    {result.error && (
                      <div className="mt-3">
                        ...
                        <pre className="rounded bg-destructive/10 p-2 font-mono text-sm text-destructive whitespace-pre-wrap overflow-x-auto">
                          {result.error}
                        </pre>
                      </div>
                    )}
